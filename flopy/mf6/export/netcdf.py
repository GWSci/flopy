...
PRECISION_STRS = ["f4", "f8", "i4", "S24", "c"]

STANDARD_VARS = ["longitude", "latitude", "layer", "delr", "delc",
                 "time"]
...
class NetCdf(object):
    ...
    def __init__(self, output_filename, model, time_values=None,
        ...
        tdis = model.simulation.get_package("TDIS")
        ...
        self.time_units = tdis.time_units.get_data()
    ...
    def initialize_geometry(self):
        ...
        grd = self.model.dimensions.get_model_grid()    
        ...
        # xmin, ymin, xmax, ymax = self.model.dis.sr.bounds
    ...
    def initialize_file(self, time_values=None):
        ...
        min_vertical = -999 #np.min(self.zs)
        max_vertical = -999 #np.max(self.zs)
        ...
        # time
        # print ("WITTW time", time_values)
        if time_values is None:
            tdis = self.model.simulation.get_package("TDIS")
            time_values = np.cumsum(self.model.tdis.perlen)
        # time_values = [self._dt_str(timedelta(days=i) +
        #                self.start_datetime) for i in np.cumsum(time_values)]
        # print ("WITTW time", time_values)
        ...
        # time = self.create_variable("time", attribs, precision_str="S24",
        #                             dimensions=("time",))
        ...
        # Elevation
        # sr = self.model.dis.sr
        # attribs = {"units": "metres", "standard_name": "elevation",
        ...
        # attribs = self.model.dis.sr.crs.grid_mapping_attribs
        ...
        # # delc
        # attribs = {"units": self.model.dis.sr.units.strip('s'),
        ...
        # delc[:] = self.model.dis.sr.delc[::-1] * self.model.dis.sr.length_multiplier
        ...
        # delr[:] = self.model.dis.sr.delr[::-1] * self.model.dis.sr.length_multiplier
        # if self.model.dis.sr.rotation != 0:
    ...
    def get_solver_H_R_tols(model):
        if model.pcg is not None:
            ...
            ims = model.simulation.get_package("IMS")
            return ims.outer_hclose.get_data(), ims.rcloserecord.get_data()[0][1]
