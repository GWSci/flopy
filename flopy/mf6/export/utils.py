...
from ..data.mfdatalist import MFList as MfList
...
def ensemble_helper(inputs_filename, outputs_filename, models, add_reals=True,
    ...
    if inputs_filename is not None:
        ...
        if i >= 2:
            if not add_reals:
                f_in.write()
                f_in = netcdf.NetCdf.empty_like(mean, output_filename=inputs_filename)
    ...
    if outputs_filename is not None:
        ...
        if i >= 2:
            if not add_reals:
                f_out.write()
                f_out = netcdf.NetCdf.empty_like(mean,
...
def _add_output_nc_variable(f, times, shape3d, out_obj, var_name, logger=None,
    ...
    for i, t in enumerate(times):
        if t in out_obj.recordarray["totim"]:
            ...
            if mask_array3d is not None and a.shape == mask_array3d.shape:
                m = ma.masked_array(data=a,
                                    mask=mask_array3d,
                                    fill_value=ma.masked)
            else:
                m = ma.masked_array(data=a)
            for mask_val in mask_vals:
                m[m == mask_val] = ma.masked
                # a[mask_array3d] = np.NaN
            try:
                array[i, :] = m.filled(netcdf.FILLVALUE).astype(np.float32)
            except Exception as e:
    ...
    # for mask_val in mask_vals:
    #     print ("WITTW mask", mask_val)
    #     array[array == mask_val] = np.NaN
    mx = np.max(array[array != netcdf.FILLVALUE])
...
def output_helper(f, ml, oudic, shape3d=None, **kwargs):
    ...
    if isinstance(f, str) and f.lower().endswith(".nc"):
        ...
        assert otimes == times
    if isinstance(f, netcdf.NetCdf) or isinstance(f, dict):
        # shape3d = (ml.dis.nlay.get_data(), ml.dis.nrow.get_data(), ml.dis.ncol.get_data())

        mask_vals = []
        mask_array3d = None
        DHNOFLO = 1.e+30
        DHDRY = -1.e+30
        mask_vals = [DHNOFLO, DHDRY]

        mask_array3d = ml.dis.idomain.get_data().flatten() == 0
...
def model_helper(f, ml, **kwargs):
    ...
    if package_names is None:
        package_names = [pak.package_name for pak in ml.packages]

    if isinstance(f, str) and f.lower().endswith(".nc"):
        f = netcdf.NetCdf(f, ml, **kwargs)
...
def package_helper(f, pak, **kwargs):
    ...
    elif isinstance(f, netcdf.NetCdf) or isinstance(f, dict):
        attrs = dir(pak)
        if 'sr' in attrs:
            attrs.remove('sr')
...
def generic_array_helper(f, array, var_name="generic_array",
    ...
    if isinstance(f, str) and f.lower().endswith(".nc"):
        ...
        f = netcdf.NetCdf(f, kwargs.pop("model"))

    assert array.ndim == len(dimensions), "generic_array_helper() " + \
                                          "array.ndim != dimensions"
    coords_dims = {"time": "time", "y": "latitude",
                   "x": "longitude"}
...
def mflist_helper(f, mfl, **kwargs):
    ...
    if isinstance(f, str) and f.lower().endswith(".shp"):
        ...
        base_name = mfl.package_name.lower()
